...
int main(){
	...
	while(!glfwWindowShouldClose(window)){
		...
		glm::mat4 transform = glm::mat4(1.0f); //initializing to identity matrix
		transform = glm::translate(transform, glm::vec3(0.0f, -0.5f, 0.0f));
		transform = glm::rotate(transform, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f)); // rotating about z axis

		// get matrix uniform location and set matrix
		myShad.use();
		unsigned int transformLoc = glGetUniformLocation(myShad.ID, "transform");
